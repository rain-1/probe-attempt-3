#!/usr/bin/env bash
# End-to-end helper for preparing current-token probe data, uploading it to
...
OUTPUT_DIR=${OUTPUT_DIR:-"data_model_current"}
BATCH_SIZE=${BATCH_SIZE:-256}
EPOCHS=${EPOCHS:-20}
HF_DATASET_ID=${HF_DATASET_ID:-""}           # e.g. username/gemma-current-probe
HF_UPLOAD_SUBDIR=${HF_UPLOAD_SUBDIR:-"current-probe"}  # folder inside HF repo
# --------------------------------------------------------------------- #
...
echo "[1/4] Preparing current-token dataset from $TEXT_FILE"
...
echo "[3/4] Training layer-0 current-token probe"
...
MODEL_PATH="models/probe_layer0_current.pth"
...
python scripts/visualize_probe.py \
  ...
  --text_file "$TEXT_FILE" \
  --num_tokens 2000 \
  --output visualization_current.html
